...
int main( int argc, char **argv )
{
...
#endif
        struct timeval startparsetime, endparsetime, startencodetime, endencodetime;
        __time_t timeuse;
        gettimeofday(&startparsetime,NULL);
        /* Parse command line */
        if( parse( argc, argv, &param, &opt ) < 0 )
                ret = -1;
        gettimeofday(&endparsetime, NULL);
        timeuse = 1000000 * (endparsetime.tv_sec - startparsetime.tv_sec) + endparsetime.tv_usec - startparsetime.tv_usec;
        printf("\nRunning Parse command line Time %luus\n", timeuse);
#ifdef _WIN32
        ...
        timeuse = 1000000 * (endencodetime.tv_sec - startencodetime.tv_sec) + endencodetime.tv_usec - startencodetime.tv_usec;
        printf("\nencode Time %luus\n", timeuse);
        /* clean up handles */
        if( filter.free )
                filter.free( opt.hin );
...
#   define  MAX_PTS_WARNING 3 /* arbitrary */
        ...
        for(; !b_ctrl_c && (i_frame < param->i_frame_total || !param->i_frame_total); i_frame++ )
        {
                ...
                timeuse = timeuse + 1000000 * (endtime.tv_sec - starttime.tv_sec) + endtime.tv_usec - starttime.tv_usec;
